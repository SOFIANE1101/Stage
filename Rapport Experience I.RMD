...
```{r}
data_Expe011 <- read.csv("C:/Users/Inspiron/Desktop/Data/SURCA_Expe01_Alldata_small.csv", 
                        header = TRUE, sep=",")
```

```{r}
data_Expe01 <- read.csv("C:/Users/Inspiron/Desktop/Data/SURCA_Expe01_Alldata.csv", 
                        header = TRUE, sep=",")
```
...
infra_Expe01 <- readOGR(dsn ="C:/Users/Inspiron/Desktop/Data/SURCA_Circuit_Expe01_210319_1058.kml",
                        layer = "Expe01_Infra", verbose = TRUE, 
                        drop_unsupported_fields=T, dropNULLGeometries=T)

VMA_small_Expe01 <- readOGR(dsn ="C:/Users/Inspiron/Desktop/Data/SURCA_Circuit_Expe01_210319_1058.kml",
                     layer = "Expe01_VMA_small", verbose = TRUE, 
                     drop_unsupported_fields=T, dropNULLGeometries=T)
```


```{r}
infra_I2V_Expe01 <- readOGR(dsn ="C:/Users/Inspiron/Desktop/Data/SURCA_Circuit_Expe01_210319_1058.kml",
                        layer = "Expe01_Infra I2V", verbose = TRUE, 
                        drop_unsupported_fields=T, dropNULLGeometries=T)
...
```{r}
coordinates_Expe01 <- as.data.frame(coordinates(infra_Expe01))
nom_infra_Expe01 <- as.data.frame(infra_Expe01@data)
coordinates_Expe01 <- coordinates_Expe01[,2:1]
```
...
suppression_plus <- function(d){
  ...
  for(i in length(d$Name):1){
    # garder le 5 et 6 eme caracter
    x <-c(substr(d$Name[i],5,6),x)
  }
  return(x)
}
nom_infra_Expe01 <-as.data.frame(suppression_plus(nom_infra_Expe01))
```
...
```{r}
liste_infra_Expe01 <- bind_cols(coordinates_Expe01,nom_infra_Expe01)
colnames(liste_infra_Expe01) <- c("Latitude","Longitude","Infra")
```

Nombre d'apparition d'une infrastructure dans le trajet de l'expérience

```{r}
infrastructures_trajet <- table(liste_infra_Expe01$Infra)%>% as.data.frame()
...
Au total `r ncol(liste_infra_Expe01)` variables pour `r nrow(liste_infra_Expe01)` enregistrements. Voici la forme du jeu de données que nous allons traiter (10 lignes aléatoires sélectionnées) 

```{r}
kable(sample_n(liste_infra_Expe01, 10), "latex", booktabs = T, linesep = "") %>%
...
```{r}
coordinates_SpeedLimit <- as.data.frame(coordinates(VMA_small_Expe01))
SpeedLimit_Expe01 <- as.data.frame(VMA_small_Expe01@data)
...
```{r, warning = FALSE, message = FALSE, results = 'hide'}
SpeedLimit_Expe01 <- subset( SpeedLimit_Expe01, select = -2 )
colnames(SpeedLimit_Expe01) <- c("SpeedLimit")

SpeedLimit_Expe01 <-separate(SpeedLimit_Expe01, SpeedLimit, 
                             c("id", "SpeedLimit", "A", "B", "C"), sep = "_")

# laisser les 2 colonnes qui contient des informations pertinante
SpeedLimit_Expe01 <- subset(SpeedLimit_Expe01, select = c(SpeedLimit, A))%>% as.data.frame()

# fusionner les 2 colonnes
SpeedLimit_Expe01$SpeedLimit <- paste(SpeedLimit_Expe01$SpeedLimit, SpeedLimit_Expe01$A)
 
#supprimer la colonne en plus
SpeedLimit_Expe01 <- subset( SpeedLimit_Expe01, select = -2)
...
SpeedLimit_Expe01[SpeedLimit_Expe01 =="Agglo DÃ©but"]<-50
SpeedLimit_Expe01[SpeedLimit_Expe01 =="Agglo Fin"]<-90
SpeedLimit_Expe01[SpeedLimit_Expe01 =="NoSign 50"]<-50
SpeedLimit_Expe01[SpeedLimit_Expe01 =="F70 Rive"] <-90
SpeedLimit_Expe01[SpeedLimit_Expe01 =="F50 Rive"] <-90
...
```{r}
liste_SpeedLimit_Expe01 <- bind_cols(SpeedLimit_Expe01,coordinates_SpeedLimit)
colnames(liste_SpeedLimit_Expe01) <- c("SpeedLimit","Latitude","Longitude")

# idee -> pour quoi renommer a chaque fois les noms des colonnes (deja renommé)
```


```{r}
ODOMETER_VMA <- c(83.6, 750, 2037, 2425, 3479, 3795, 4227, 5795, 
       2931.8, 4170.7, 4973, 6203.7, 
       4459.5, 4972.8, 6382,
       3822.9, 5208.3, 5561.1,
       2758.7, 2850.7, 5041,
       320.1, 625.6, 1271.4, 1656.2, 5444.4, 5500.4, 5855.9, 6145.6,
       103.1, 1909.5, 2090.1, 2381, 3730.6, 4859, 4986.1,
       5370.6, 6150.1, 6245.2,
       215.4, 1075.6, 3983.5, 4727, 4912.5, 5912.8,
       3779.2, 5159.3, 6382.2,
       540.2, 727.7, 888.1, 1936.8, 3886.3, 4524.7, 5230.9, 5527.4,
       1704.6, 2430.5, 2640.9, 5080.3)
```

```{r}
Distance_cum_VMA <- c(83.6, 750, 2037, 2425, 3479, 3795, 4227, 5795,
       9482.5, 10721.4, 11523.7, 12754.4,
       17563.7, 18077, 19486.2,
       23480.6, 24866, 25218.8,
       35523.4, 35615.4, 37805.7,
       39638.3, 39943.8, 40589.6, 40974.4, 44762.6, 44818.6, 45174.1, 45463.8,
       45974.8, 47781.2, 47961.8, 48252.7, 49602.3, 50730.7, 50857.8,
       51242.3, 52021.8, 52116.9,
       52640.6, 53500.8, 56408.7, 57152.2, 57337.7, 58338,
       62757.9, 64138, 65360.9,
       66072.4, 66259.9, 66420.3, 67469, 69418.5, 70056.9, 70763.1, 71059.6,
       73790.3, 74516.2,74726.6, 77166)
```

```{r}
liste_SpeedLimit_Expe01 <- bind_cols(liste_SpeedLimit_Expe01, ODOMETER_VMA, Distance_cum_VMA)
colnames(liste_SpeedLimit_Expe01) <- c("SpeedLimit","Latitude","Longitude","ODOMETER_VMA", "Distance_cum_VMA")
```
...
```{r}
kable(sample_n(liste_SpeedLimit_Expe01, 10), "latex", booktabs = T, linesep = "") %>%
  kable_styling(position = "center",latex_options = "HOLD_position")
```
...
```{r, echo = FALSE, fig.width=10, fig.height=6}
Speed_Limit_trajet <- table(liste_SpeedLimit_Expe01$SpeedLimit)%>% as.data.frame()
...
```{r}
# LATITUDE, LONGITUDE sont à 0 (s01 et s)
# ODOMETER et Distance_cum sont à 0 (S32)
#

data_Expe011 <- data_Expe011  %>% dplyr::select (DriverID,
                                               Timestamp,ODOMETER,Distance_cum, 
                                               LATITUDE, LONGITUDE)
...
AI <- data_Expe011  %>% dplyr::select (DriverID,ODOMETER,Distance_cum)
AII <- data_Expe011  %>% dplyr::select (DriverID,LATITUDE, LONGITUDE)

```

```{r}
A <- subset(AI,DriverID == "S01")
B <- subset(AI,DriverID == "S02")
```

#####################
```{r}
x <- c(83.6, 750, 2037, 2425, 3479, 3795, 4227, 5795,  )
```

###############################################################


Au total `r ncol(data_Expe01)` variables pour `r nrow(data_Expe01)` enregistrements. Voici la forme du jeu de données que nous allons traiter (10 lignes aléatoires sélectionnées) :

```{r}
kable(sample_n(data_Expe01, 10), "latex", booktabs = T, linesep = "") %>%
  kable_styling(position = "center",latex_options = "HOLD_position")
```
\newpage
...
#data_Expe011$timestamp <- calcule_timestamp(data_Expe011)
data_S02$Timestamp <- calcule_timestamp(data_S02)
```
...
```{r}
indice_debut_conducteur <-debut_conducteur(data_Expe01)
```
...
Colonne_NumRecord <- function(d,lst_indice){
  ...
  # on ajoute l'indice de la dernière ligne
  Record2 <- c(Record2,Record2[length(Record2)]+1)
  setTxtProgressBar(pb, j)
  return(Record2)
}
data_Expe011$NumRecord <- Colonne_NumRecord(data_Expe011,indice_debut_conducteur)
data_Expe01$NumRecord <- Colonne_NumRecord(data_Expe01,indice_debut_conducteur)
```
...
```{r}
colnames(data_Expe04) <- c("DriverID", "InstantSpeed_CAN", "Timestamp","Distance_cum",
...
pb <- txtProgressBar(min = 1, max = nrow(d2) , style = 3)
...
data_util <- d1[( ((d2$dis_cum_VMA[i]-100) < d1$Odometre_Pk) & (d1$Odometre_Pk< (d2$dis_cum_VMA[i]+100)) ),]
...
# on ajoute la VMA à d1
d1$SpeedLimit[(d1$Odometre_Pk==data_util$Odometre_Pk[indice])] <- d2$SpeedLimit[i]
...
d1$SpeedLimit <- na.locf(d1$SpeedLimit,fromLast=F,na.rm=F) # Complète les NA par la VMA d'avant
d1$SpeedLimit <- na.locf(d1$SpeedLimit,fromLast=T,na.rm=F) # Complète les NA par l'arrière
